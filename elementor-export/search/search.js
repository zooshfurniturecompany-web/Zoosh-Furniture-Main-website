document.addEventListener("DOMContentLoaded", function() {
  ...
  const queryTitle = document.getElementById("zooshSearchQueryTitle");
  const queryDesc = document.getElementById("zooshSearchQueryDesc");
  const productGrid = document.getElementById("zooshSearchGrid");
  const noResultsPanel = document.getElementById("zooshSearchNoResults");

  // Read URL query parameter "?q=dining" or "?search=dining"
  const urlParams = new URLSearchParams(window.location.search);
  const searchQuery = urlParams.get("q") || urlParams.get("search") || "";

  // Update headers
  if (queryTitle) {
    queryTitle.textContent = searchQuery ? `Search Results for "${searchQuery}"` : "Search Results";
  }
  ...
  function renderSearchResults() {
    ...
    if (filtered.length > 0) {
      ...
      if (queryDesc) {
        queryDesc.textContent = `Showing ${filtered.length} matching item(s) from our solid wood catalog.`;
      }
      ...
      if (queryDesc) {
        queryDesc.textContent = searchQuery ? `No matches found for "${searchQuery}".` : "Enter a search term in the header to search our catalog.";
      }
    }
  }
